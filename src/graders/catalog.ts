...
export const authUIChecks = registerJudges({
  ...
  usesSignedIn:
    'Does the solution use the <Show when="signed-in"> component to conditionally render content for authenticated users, without using the removed <SignedIn> component?',
  usesSignedOut:
    'Does the solution use the <Show when="signed-out"> component to conditionally render content for unauthenticated users, without using the removed <SignedOut> component?',
})
