...
function parseRateLimitConfig(): Map<ProviderKey, number> {
  ...
  const deepseekRPM = Number.parseInt(
    process.env.DEEPSEEK_RATE_LIMIT_RPM || String(DEFAULT_RATE_LIMITS.deepseek),
    10,
  )
  delays.set('deepseek', Math.ceil(60000 / deepseekRPM))
