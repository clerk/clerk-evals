...
export const graders = {
  ...
  handles_membership_created: async (actual) =>
    actual.includes("organizationMembership.created"),
  handles_membership_deleted: async (actual) =>
    actual.includes("organizationMembership.deleted"),
  logs_membership_id: async (actual) =>
    actual.includes("evt.data.id") || actual.includes("membership.id"),
  logs_org_id: async (actual) =>
    actual.includes("evt.data.organization.id") ||
    actual.includes("organizationId"),
  references_public_user_data: async (actual) =>
    actual.includes("publicUserData"),
