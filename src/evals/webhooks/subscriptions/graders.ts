...
export const graders = {
  ...
  handles_subscription_created: async (actual) => actual.includes('subscription.created'),
  logs_payer_id: async (actual) =>
    actual.includes('evt.data.payerId') || actual.includes('payerId'),
  handles_subscription_past_due: async (actual) => actual.includes('subscription.pastDue'),
  logs_subscription_id: async (actual) => actual.includes('evt.data.id'),
  errors_on_past_due: async (actual) =>
    actual.includes('console.error') && actual.includes('subscription.pastDue'),
  references_status_field: async (actual) => actual.includes('evt.data.status'),
