...
export const graders = {
  ...
  handles_email_created: async (actual) => actual.includes('email.created'),
  logs_email_payload: async (actual) =>
    actual.includes('console.log') &&
    actual.includes('email.created') &&
    actual.includes('JSON.stringify'),
  handles_sms_created: async (actual) => actual.includes('sms.created'),
  warns_sms_payload: async (actual) =>
    actual.includes('console.warn') &&
    actual.includes('sms.created') &&
    actual.includes('JSON.stringify'),
  mentions_event_id: async (actual) => actual.includes('evt.data.id') || actual.includes('eventId'),
