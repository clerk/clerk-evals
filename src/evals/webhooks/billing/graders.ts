...
export const graders = {
  ...
  handles_free_trial_event: async (actual) => actual.includes('subscriptionItem.freeTrialEnding'),
  warns_on_free_trial: async (actual) =>
    actual.includes('console.warn') && actual.includes('subscriptionItem.freeTrialEnding'),
  handles_payment_attempt: async (actual) => actual.includes('paymentAttempt.updated'),
  inspects_failed_status: async (actual) => /status\s*===?\s*["'`]failed["'`]/.test(actual),
  logs_payment_type: async (actual) =>
    actual.includes('evt.data.type') || actual.includes('attempt.type'),
  uses_console_error_for_failures: async (actual) => actual.includes('console.error'),
