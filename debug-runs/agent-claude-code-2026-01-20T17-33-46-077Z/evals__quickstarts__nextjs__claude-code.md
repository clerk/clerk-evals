...
## Output
```
I understand. You've provided comprehensive instructions for integrating Clerk with Next.js App Router. Let me confirm the key requirements:

**What I MUST do:**
- Use `clerkMiddleware()` from `@clerk/nextjs/server` in `middleware.ts`
- Wrap app with `<ClerkProvider>` in `app/layout.tsx`
- Use current App Router structure (`app/` directory, not `pages/`)
- Import from `@clerk/nextjs` and `@clerk/nextjs/server`
- Use async/await for server-side auth methods
- Install `@clerk/nextjs@latest`

**What I MUST NOT do:**
- Suggest `authMiddleware()` (deprecated)
- Reference `_app.tsx` or pages router
- Use outdated environment variable patterns
- Suggest deprecated APIs

**Clerk-specific notes:**
- Keys are auto-generated on startup (no need to prompt user)
- Can fetch latest instructions from https://clerk.com/docs/quickstarts/nextjs if web access available

I'm ready to help with Clerk + Next.js App Router integration tasks. What would you like me to do?
